...
export default async (props: Props): Promise<ApiResponse> => {
  ...
  const valid_name = isValidObjectName(file_name);
  if (valid_name.error) return { ...BAD, message: `${valid_name.message}` };
  ...
  try {
    ...
    if (object_exists.status === OK.status) CONFLICT;

    const client = getMinioClient();
    const object_info = await client.putObject(bucket_name, file_name, file_stream, file_size, metaData);
    if (!object_info) throw new Error("Filed to upload form object.");
